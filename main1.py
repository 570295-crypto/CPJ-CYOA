...
class Alchemist(Combatant):
    ...
    def chill_touch(self, target):
        ...
        dealt = target.take_damage(damage, self)
        target.effects["Chilled"] = 2
        print(f"{self.name} casts Chill Touch on {target.name}, dealing {dealt} damage and applying Chilled. {target.name} has {target.health} health remaining.")
        return True
...
class Paladin(Combatant):
    ...
    def decapitate(self, target):
        ...
        if dealt > 0:
            ...
            print(f"{self.name} casts Decapitate on {target.name}, but it missed. {target.name} has {target.health} health remaining.")
        return True
...
class Assassin(Combatant):
    ...
    def knife_dance(self):
        self.effects["Focused"] = 2
        self.effects["Dodge"] = self.knife_dance_dodge_rounds + 1
...
class Marksman(Combatant):
    ...
    def triple_bolt(self, target):
        ...
        if dealt > 0 and random.random() < 0.45:
            ...
            stunned = True
        if stunned:
            print(f"{self.name} fires three bolts at {target.name}, dealing {dealt} damage and stunning them. {target.name} has {target.health} health remaining.")
        else:
